...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-900">
      <header className="bg-slate-950 border-b border-slate-800 px-4 lg:px-6 py-3 flex items-center justify-between sticky top-0 z-[1000]">
      ...
      <main className="max-w-7xl mx-auto px-4 lg:px-6 py-5 space-y-5">
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
            ...
            {selectedZone && (
              <div className="mt-4 bg-slate-900 border border-slate-700 rounded-xl p-4">
                <h3 className="text-white font-bold text-base mb-2">{selectedZone.name}</h3>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-white font-bold text-lg">Claims Queue</h2>
                ...
                <div key={claim.id} className="bg-slate-900 border border-slate-700 rounded-xl overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-white font-semibold text-sm">{claim.zone || claim.zone_id}</p>
                        <p className="text-slate-400 text-xs">{claim.date} · {claim.confidence} · ₹{(claim.recommendedPayout || claim.recommended_payout || 0).toLocaleString()}</p>
                      </div>
                    </div>
